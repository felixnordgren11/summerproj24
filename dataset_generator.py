...
image_folders = ['layer_one_images', 'avg_images']


start = 1
step = 0.004
end = 1.4
amount = np.ceil((end-start)/step)-1
dm_scale_values = [round(start + step*i, 3) for i in range(int(amount)+1)]
...
# simulation result files that are not needed. They are not to be removed before the 
# image generator has run.
for value in dm_scale_values:
    ...
    #Write over the old file with the new values.
    with open(original_file_path, 'w') as file:
